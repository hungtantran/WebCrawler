...
public class HttpFetcher implements IHttpFetcher {
    private static Logger LOG = LogManager.getLogger(HttpFetcher.class
            .getName());
    ...
    private static final String[] userAgents = {
            ...
            "Mozilla/5.0 (Macintosh; U; PPC Mac OS X Mach-O; en-US; rv:1.7a) " +
                    "Gecko/20050614 Firefox/0.9.0+",
            "Mozilla/5.0 (Macintosh; U; PPC Mac OS X; en-US) " +
                    "AppleWebKit/125.4 (KHTML, like Gecko, Safari) " +
                    "OmniWeb/v563.15"
    };
    ...
    public CrError getWebPage(URLObject inUrl, IWebPage downloadedWebPage,
                              int numRetries) throws Exception {
            ...
            try {
                ...
                Connection connection = Jsoup
                        ...
                        .header("Accept-Language", "en-US,en;q=0.8,de;q=0.6," +
                                "vi;q=0.4")
                        .header("Cookie", "cl_b=mAqo5vfg5BG2A_Lt3WzWGw2dXR8; " +
                                "cl_def_lang=en; cl_def_hp=detroit; " +
                                "cl_tocmode=sss%3Agrid%2Csso%3Alist%2Chhh" +
                                "%3Alist")
                        .timeout(10000);

                Response response = connection.execute();
                LOG.info("Download successfully link " + url + " after " + i
                        + " retries with user agent " + HttpFetcher
                        .userAgents[ranIndex]);

                long duration = Helper.getCurrentTimeInMillisec() - startTime;
                LOG.info("Duration = " + duration + " start time = " +
                        startTime + " current time = " + Helper
                ...
                if (i == numRetries - 1) {
                    if (e instanceof HttpStatusException) {
                        ...
                        inUrl.set_httpStatusCode(statusException
                                .getStatusCode());
                        LOG.error("Fail to download link " + url + " after "
                                + i + " retries with user agent " +
                                HttpFetcher.userAgents[ranIndex] + " with " +
                                "status code " + statusException
                                .getStatusCode());
                    } else {
                        LOG.error("Fail to download link " + url + " after "
                                + i + " retries with user agent " +
                                HttpFetcher.userAgents[ranIndex]);
