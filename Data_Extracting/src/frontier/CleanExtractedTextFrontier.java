...
public class CleanExtractedTextFrontier {
    ...
    public CleanExtractedTextFrontier(IDatabaseConnection databaseConnection, int lowerBound, int maxCount) {
        LOG.setLevel(Level.ALL);
        m_databaseConnection = databaseConnection;
        m_lowerBound = lowerBound;
        m_maxCount = maxCount;
        LOG.info("Starting lowerbound = " + m_lowerBound + ", maxCount = " + m_maxCount);
    }

    public ErrorCode.CrError pullCleanExtractedText(ExtractedText extractedText) {
        synchronized (m_backEndQueues) {
            if (m_backEndQueues.isEmpty()) {
                try {
                    ...
                    m_backEndQueues = m_databaseConnection.getCleanExtractedText(lowerBound, count);
                    m_totalCount += count;
                    LOG.info("LowerBound = " + lowerBound + ", count = " + count);
